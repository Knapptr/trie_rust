...
impl NodeList {
    ...
    fn add_char(&mut self, char: char, terminal: bool) {
        match &mut self.nodes.get_mut(&char) {
            Some(child) => {
                if child.terminal {
                    return;
                }
            ...
            None => {
                self.nodes.insert(char, Node::new(char, terminal));
            }
        }
    }
    fn add_word(&mut self, word: &str) {
        ...
        for (i, char) in word.chars().enumerate() {
            if i == length - 1 {
                ...
                match current_list.nodes.get_mut(&char) {
    ...
    fn existing_nodes(&self) -> Option<Vec<&Node>> {
        let children: Vec<&Node> = self.nodes.iter().map(|(_, node)| node).collect();
        if children.len() == 0 {
            return None;
        }
        Some(children)
    ...
    fn find_starts_with(&self, str: &str) -> Vec<String> {
        ...
        for char in str.chars() {
            if let Some(current_node) = current_index.nodes.get(&char) {
